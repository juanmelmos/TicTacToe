...
public class WThreadRankingMedio extends Thread {
	...
	public WThreadRankingMedio(JTable table) {
		...
		while (true) {
			...
			try {
				...
				if(rs.next()) {
					...
					select= "select Usuario, VictoriasLocalMedio, DerrotasLocalMedio, EmpatesLocalMedio from Usuarios order by VictoriasLocalMedio desc, DerrotasLocalMedio asc;";
					//Aws.connect(false);
					ps = Aws.getConnection().prepareStatement(select);
					rs2 = ps.executeQuery();

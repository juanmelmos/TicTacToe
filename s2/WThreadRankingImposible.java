...
public class WThreadRankingImposible extends Thread {
	...
	protected static Object matrizObjetos[][];
	...
	public WThreadRankingImposible(JTable table) {
		...
		while (true) {
			...
			try {
				...
				if(rs.next()) {
					...
					select= "select Usuario, VictoriasLocalImposible, DerrotasLocalImposible, EmpatesLocalImposible from Usuarios order by VictoriasLocalImposible desc, DerrotasLocalImposible asc;";
					//Aws.connect(false);
					ps = Aws.getConnection().prepareStatement(select);
					rs2 = ps.executeQuery();

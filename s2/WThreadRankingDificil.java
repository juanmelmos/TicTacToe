...
public class WThreadRankingDificil extends Thread {
	...
	public WThreadRankingDificil(JTable table) {
		...
		while (true) {
			...
			try {
				...
				if(rs.next()) {
					...
					select= "select Usuario, VictoriasLocalDificil, DerrotasLocalDificil, EmpatesLocalDificil from Usuarios order by VictoriasLocalDificil desc, DerrotasLocalDificil asc;";
					//Aws.connect(false);
					ps = Aws.getConnection().prepareStatement(select);
					rs2 = ps.executeQuery();

...
public class WThreadRankingFacil extends Thread {
	...
	public WThreadRankingFacil(JTable table) {
		...
		while (true) {
			...
			try {
				...
				if(rs.next()) {
					...
					select= "select Usuario, VictoriasLocalFacil, DerrotasLocalFacil, EmpatesLocalFacil from Usuarios order by VictoriasLocalFacil desc, DerrotasLocalFacil asc;";
					//Aws.connect(false);
					ps = Aws.getConnection().prepareStatement(select);
					rs2 = ps.executeQuery();
